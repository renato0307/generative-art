...
function draw() {	
	...
	for (var x = offset; x < size - offset; x += tileStep) {
		for (var y = offset; y < size - offset; y += tileStep) {
			...
			drawRecursive(x, y, startSize, startSize, xDirection, yDirection, startSteps - 1);
		}
	}
}

function drawRecursive(x, y, width, height, xMovement, yMovement, steps) {
	...
	if (steps >= 0) {
		...
		drawRecursive(newX, newY, newSize, newSize, xMovement, yMovement, steps - 1);
	}
}
